    ...
    return {
        ...
        async load(id: string) {
            if (id === resolvedVirtualModuleId) {
                ...
                for (const [componentName, componentFile] of Object.entries(components)) {
                    const resolvedId = await this.resolve(
                        "/" + componentsDir + "/" + componentFile + ".astro"
                    );

                    if (resolvedId) {
                        exports.push(`export { default as ${camelcase(componentName)} } from "${resolvedId.id}"`)
                    }
                }
                ...
                if (fallbackComponentResolvedId) {
                    ...
                    exports.push(`export { default as fallback } from "@flyo/nitro-astro/components/FallbackComponent.astro"`)
                }
